...
*Implementation introduced in DRF-extensions 0.2.4 is deprecated and unsupported*
...
There is few things you need to take care in viewsets which are used in nested routers.
They should add mixin `NestedViewSetMixin`, which adds automatic filtering by parent lookups.
...
In addition you need to define `parent_lookup_map` for `NestedViewSetMixin` to know how to use url parameters.
    ...
    class PermissionViewSet(NestedViewSetMixin, ModelViewSet):
        ...
        parent_lookup_map = {'user_id': 'group.user.id', 'group_id': 'group.id'}
...
With such kind of router and `lookup_url_kwarg = 'permission_id'` in `PermissionViewSet` we would have following resources:
